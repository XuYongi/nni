...
def replace_instancenorm2d(norm, mask):
    """
    Parameters
    ----------
    norm : torch.nn.InstanceNorm2d
        The InstanceNorm2d module to be replace
    mask : ModuleMasks
        The masks of this module
    ...
    torch.nn.InstanceNorm2d
        The new InstanceNorm2d module
    """
    assert isinstance(mask, ModuleMasks)
    index = mask.param_masks['weight'].mask_index[0]
    num_features = index.size()[0]
    _logger.debug("replace InstanceNorm2d with num_features: %d", num_features)
...
def replace_conv2d(conv, mask):
    ...
    if mask.input_mask is not None:
        for groupid in range(conv.groups):
            ...
            print("current_input_index",current_input_index,in_channels_index)
...
def replace_convtranspose2d(conv, mask):
    """
    Parameters
    ----------
    conv : torch.nn.ConvTranspose2d
        The ConvTranspose2d module to be replaced
    mask : ModuleMasks
        The masks of this module
    ...
    torch.nn.ConvTranspose2d
        The new ConvTranspose2d module
    """
    assert isinstance(mask, ModuleMasks)
    if mask.input_mask is None:
        in_channels = conv.in_channels
    ...
    if mask.output_mask is None:
        ...
        out_channels = out_channels_index.size()[0]
    _logger.debug("replace conv2d with in_channels: %d, out_channels: %d", in_channels, out_channels)
    new_conv = torch.nn.ConvTranspose2d(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=conv.kernel_size,
                               stride=conv.stride,
                               padding=conv.padding,
    ...
    if mask.input_mask is not None:
        for groupid in range(conv.groups):
            ...
            f_start = groupid * filter_step
            f_end = (groupid + 1) * filter_step
            new_conv.weight.data[:] = torch.index_select(tmp_weight_data[:,f_start:f_end], 0, current_input_index)
    else:
        new_conv.weight.data.copy_(tmp_weight_data)
